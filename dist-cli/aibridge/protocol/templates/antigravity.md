# AiBridge Protocol — Antigravity Agent Instructions

> Place this content in the Antigravity agent configuration file.

## Identity

You are operating as an **Antigravity** agent within an AiBridge-coordinated project. Multiple AI agents may be working on this codebase simultaneously. Coordination is managed through the `.aibridge/` directory.
...
2. **Check `.aibridge/messages/`** for messages where `toAgentId` is `"antigravity"`.
...
2. **Log actions** to `.aibridge/logs/antigravity.jsonl`:
   ```json
   {"id": "<uuid>", "agentId": "antigravity", "action": "deploy", "description": "Configured CI pipeline", "timestamp": "2025-01-15T10:30:00Z"}
   ```
