...
function getBridgePaths(root: string): BridgePaths {
  const repoRoot = getRepoPath(root);
  return {
    root,
    repoRoot,
...
async function writeTextAtomic(targetPath: string, value: string) {
  await ensureDir(path.dirname(targetPath));
  const existingValue = await fs.readFile(targetPath, "utf8").catch(() => null);
...
async function regenerateContextUnlocked(rootPath: string, budget?: number) {
  ...
  await syncInstructionArtifactsUnlocked(rootPath);
  return markdown;
}
...
async function writeGeneratedFile(targetPath: string, value: string) {
  await ensureDir(path.dirname(targetPath));
  await writeTextAtomic(targetPath, `${value.trimEnd()}\n`);
}

function cursorRuleMarkdown(template: string) {
  return [
    "---",
    "description: AiBridge coordination protocol",
    "alwaysApply: true",
    "---",
    "",
    template.trim(),
...
async function syncInstructionArtifactsUnlocked(rootPath: string) {
  const snapshot = await loadBridgeSnapshot(rootPath);
  const paths = getBridgePaths(rootPath);
  const cursorTemplate = await readTextIfExists(path.resolve(PROTOCOL_TEMPLATES_ROOT, "cursor.md"));
  const codexTemplate = await readTextIfExists(path.resolve(PROTOCOL_TEMPLATES_ROOT, "codex.md"));
  const generatedNotice = "<!-- Generated by AiBridge. Regenerated from the local runtime. -->";

  await writeGeneratedFile(
    path.join(paths.repoRoot, ".cursorrules"),
    [generatedNotice, "", cursorTemplate || "# AiBridge Cursor Protocol"].join("\n"),
  );

  await writeGeneratedFile(
    path.join(paths.repoRoot, ".cursor", "rules", "aibridge.mdc"),
  ...
  const antigravitySection = snapshot.bridge.agents.some((agent) => agent.kind === "antigravity")
    ? [
        ...
        "## Antigravity Adapter",
        "",
        "- Use `.aibridge/agents/antigravity.md` as the Antigravity-specific role file.",
        "- Treat `AGENTS.md` as supportive context and the AiBridge launch prompt as primary.",
      ].join("\n")
    : "";

  await writeGeneratedFile(
    path.join(paths.repoRoot, "AGENTS.md"),
